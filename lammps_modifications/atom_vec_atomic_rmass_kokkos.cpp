...
#include "atom_vec_atomic_rmass_kokkos.h"
...
AtomVecAtomicRmassKokkos::AtomVecAtomicRmassKokkos(LAMMPS *lmp) : AtomVec(lmp),
AtomVecKokkos(lmp), AtomVecAtomicRmass(lmp)
{
...
void AtomVecAtomicRmassKokkos::grow(int n)
{
...
void AtomVecAtomicRmassKokkos::grow_pointers()
{
  tag = atomKK->tag;
...
void AtomVecAtomicRmassKokkos::sort_kokkos(Kokkos::BinSort<KeyViewType, BinOp> &Sorter)
{
...
struct AtomVecAtomicRmassKokkos_PackBorder {
  ...
  AtomVecAtomicRmassKokkos_PackBorder(
...
int AtomVecAtomicRmassKokkos::pack_border_kokkos(
  ...
  int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    if (space==Host) {
      AtomVecAtomicRmassKokkos_PackBorder<LMPHostType,1> f(
        buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
        h_x,h_tag,h_type,h_mask,h_rmass,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecAtomicRmassKokkos_PackBorder<LMPDeviceType,1> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        d_x,d_tag,d_type,d_mask,d_rmass,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
    ...
    dx = dy = dz = 0;
    if (space==Host) {
      AtomVecAtomicRmassKokkos_PackBorder<LMPHostType,0> f(
        buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
        h_x,h_tag,h_type,h_mask,h_rmass,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecAtomicRmassKokkos_PackBorder<LMPDeviceType,0> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        d_x,d_tag,d_type,d_mask,d_rmass,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
...
struct AtomVecAtomicRmassKokkos_PackBorderVel {
  typedef DeviceType device_type;
  ...
  AtomVecAtomicRmassKokkos_PackBorderVel(
    const typename ArrayTypes<DeviceType>::t_xfloat_2d &buf,
    const typename ArrayTypes<DeviceType>::t_int_1d_const &list,
    const typename ArrayTypes<DeviceType>::t_x_array &x,
    const typename ArrayTypes<DeviceType>::t_tagint_1d &tag,
...
int AtomVecAtomicRmassKokkos::pack_border_vel_kokkos(
  int n, DAT::tdual_int_1d k_sendlist, DAT::tdual_xfloat_2d buf,
  int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    if (!deform_vremap) {
      if (space==Host) {
        AtomVecAtomicRmassKokkos_PackBorderVel<LMPHostType,1,0> f(
          buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
        ...
        AtomVecAtomicRmassKokkos_PackBorderVel<LMPDeviceType,1,0> f(
          buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
      ...
      dvz = pbc[2]*h_rate[2];
      if (space==Host) {
        AtomVecAtomicRmassKokkos_PackBorderVel<LMPHostType,1,1> f(
          buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
          h_x,h_tag,h_type,h_mask,
          h_rmass,
          h_v,
        ...
        AtomVecAtomicRmassKokkos_PackBorderVel<LMPDeviceType,1,1> f(
          buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
          d_x,d_tag,d_type,d_mask,
          d_rmass,
          d_v,
    ...
    if (space==Host) {
      AtomVecAtomicRmassKokkos_PackBorderVel<LMPHostType,0,0> f(
        buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
        h_x,h_tag,h_type,h_mask,
        h_rmass,
        h_v,
      ...
      AtomVecAtomicRmassKokkos_PackBorderVel<LMPDeviceType,0,0> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        d_x,d_tag,d_type,d_mask,
        d_rmass,
        d_v,
...
struct AtomVecAtomicRmassKokkos_UnpackBorder {
  typedef DeviceType device_type;
  ...
  AtomVecAtomicRmassKokkos_UnpackBorder(
      ...
      const typename ArrayTypes<DeviceType>::t_float_1d &rmass,
      const int& first):
      _buf(buf),_x(x),_tag(tag),_type(type),_mask(mask),_rmass(rmass),_first(first){};
...
void AtomVecAtomicRmassKokkos::unpack_border_kokkos(const int &n, const int &first,
                                               const DAT::tdual_xfloat_2d &buf,ExecutionSpace space) {
  while (first+n >= nmax) grow(0);
  if (space==Host) {
    struct AtomVecAtomicRmassKokkos_UnpackBorder<LMPHostType> f(buf.view<LMPHostType>(),
      h_x,h_tag,h_type,h_mask,
      h_rmass,
      first);
    Kokkos::parallel_for(n,f);
  } else {
    struct AtomVecAtomicRmassKokkos_UnpackBorder<LMPDeviceType> f(buf.view<LMPDeviceType>(),
      d_x,d_tag,d_type,d_mask,
      d_rmass,
      first);
    Kokkos::parallel_for(n,f);
...
template<class DeviceType>
struct AtomVecAtomicRmassKokkos_UnpackBorderVel {
  typedef DeviceType device_type;
  ...
  AtomVecAtomicRmassKokkos_UnpackBorderVel(
    const typename ArrayTypes<DeviceType>::t_xfloat_2d_const &buf,
    const typename ArrayTypes<DeviceType>::t_x_array &x,
    const typename ArrayTypes<DeviceType>::t_tagint_1d &tag,
    const typename ArrayTypes<DeviceType>::t_int_1d &type,
...
void AtomVecAtomicRmassKokkos::unpack_border_vel_kokkos(
  const int &n, const int &first,
  const DAT::tdual_xfloat_2d &buf,ExecutionSpace space) {
  while (first+n >= nmax) grow(0);
  if (space==Host) {
    struct AtomVecAtomicRmassKokkos_UnpackBorderVel<LMPHostType> f(buf.view<LMPHostType>(),
      h_x,h_tag,h_type,h_mask,
      h_rmass,
      h_v,
      first);
    ...
    struct AtomVecAtomicRmassKokkos_UnpackBorderVel<LMPDeviceType> f(buf.view<LMPDeviceType>(),
      d_x,d_tag,d_type,d_mask,
      d_rmass,
      d_v,
      first);
  ...
  atomKK->modified(space,X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|RMASS_MASK|V_MASK);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
struct AtomVecAtomicRmassKokkos_PackExchangeFunctor {
  typedef DeviceType device_type;
  ...
  AtomVecAtomicRmassKokkos_PackExchangeFunctor(
...
int AtomVecAtomicRmassKokkos::pack_exchange_kokkos(
  const int &nsend,
  ...
  ExecutionSpace space)
{
  ...
  if (nsend > (int) (k_buf.view<LMPHostType>().extent(0)*k_buf.view<LMPHostType>().extent(1))/size_exchange) {
    int newsize = nsend*13/k_buf.view<LMPHostType>().extent(1)+1;
    k_buf.resize(newsize,k_buf.view<LMPHostType>().extent(1));
  }
  atomKK->sync(space,X_MASK | V_MASK | TAG_MASK | TYPE_MASK |
             MASK_MASK | IMAGE_MASK| RMASS_MASK);

  if (space == Host) {
    AtomVecAtomicRmassKokkos_PackExchangeFunctor<LMPHostType> f(atomKK,k_buf,k_sendlist,k_copylist);
    Kokkos::parallel_for(nsend,f);
  } else {
    AtomVecAtomicRmassKokkos_PackExchangeFunctor<LMPDeviceType> f(atomKK,k_buf,k_sendlist,k_copylist);
    Kokkos::parallel_for(nsend,f);
  }
...
struct AtomVecAtomicRmassKokkos_UnpackExchangeFunctor {
  typedef DeviceType device_type;
  typedef ArrayTypes<DeviceType> AT;
  ...
  AtomVecAtomicRmassKokkos_UnpackExchangeFunctor(
    const AtomKokkos* atom,
    const typename AT::tdual_xfloat_2d buf,
...
int AtomVecAtomicRmassKokkos::unpack_exchange_kokkos(DAT::tdual_xfloat_2d &k_buf, int nrecv, int nlocal,
                                                ...
                                                DAT::tdual_int_1d &k_indices)
{
  ...
  if (space == Host) {
    ...
    if (k_indices.h_view.data()) {
      AtomVecAtomicRmassKokkos_UnpackExchangeFunctor<LMPHostType,1> f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
    } else {
      AtomVecAtomicRmassKokkos_UnpackExchangeFunctor<LMPHostType,0> f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
    }
  } else {
    k_count.h_view(0) = nlocal;
    ...
    if (k_indices.h_view.data()) {
      AtomVecAtomicRmassKokkos_UnpackExchangeFunctor<LMPDeviceType,1> f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
    } else {
      AtomVecAtomicRmassKokkos_UnpackExchangeFunctor<LMPDeviceType,0> f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
    }
...
void AtomVecAtomicRmassKokkos::sync(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
...
void AtomVecAtomicRmassKokkos::sync_overlapping_device(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
...
void AtomVecAtomicRmassKokkos::modified(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
